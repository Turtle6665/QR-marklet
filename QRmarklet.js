javascript: 
/*Share URL as QRcode popup from https://github.com/Turtle6665/QRmarklet*/
function QRmarklet() {
  let qRPopup = document.createElement("div");
  Object.assign(qRPopup, {
    style:
      "z-index:9999999; text-align: center; background-color: #202020; position: fixed; top: 0; left: 0; padding: 1em; border-radius:1em; border: 1px solid white;",
  });

  let qRText = document.createElement("textarea");
  let qRQuit = document.createElement("button");
  let qRContainer = document.createElement("div");
  function updateQRCode(text) {
    qRContainer.innerHTML = "";
    qRContainer.appendChild(new QRCode({ msg: text, pal: ["#000", "#fff"] }));
  }

  qRPopup.appendChild(qRText);
  Object.assign(qRText, {
    style:
      "background-color: #202020; color: #F8F8FF ; vertical-align: middle;",
  });
  qRText.onkeyup = function () {
    updateQRCode(this.value);
  };
  qRText.value = window.location.href;
  updateQRCode(qRText.value);

  qRPopup.appendChild(qRQuit);
  Object.assign(qRQuit, {
    onclick: function () {
      qRPopup.remove();
    },
    innerText: "x",
    style:
      "background-color: #202020; width:2em; height: 2em;color: #F8F8FF; padding:0.2em; margin-left: 0.2em; border-radius:0.2em;",
  });

  qRPopup.appendChild(qRContainer);

  document.children[0].appendChild(qRPopup);
}

/** QRCode() from https://github.com/datalog/qrcode-svg under MIT license */
